...
- 2026-05-18: Closed silent-skip gap on evaluator/fact-check remarks. Incident: lock-free page `concurrent-data-structures.mdx` (commit 3d76cea) shipped with 5 unresolved ⚠️ items from `fact-check/concurrent-data-structures.md` (Mars Pathfinder mutex framing; ConcurrentSkipListMap "javadoc" attribution; NetBSD `vmem` vs `pserialize` for EBR; SPAA 2004 Sun E6500 14-CPU vs 1..32 axis on contention chart; ref [19] `lwlock.c` vs `xlog.c` location of `NUM_XLOGINSERT_LOCKS`). Root cause: no enforcement that every ⚠️/❌/🔍 row in the fact-check log must be resolved before approval. Fix (single gate, not duplicated across agents per user direction): added blocking "Remarks gate" to evaluator with three-way (FIXED / DEFERRED-with-ISSUES.md / OUT-OF-SCOPE-with-justification) trichotomy, plus mandatory `Unresolved remarks: N` and `Resolution ledger:` lines in the evaluator report; `approve` is invalid unless N = 0.
