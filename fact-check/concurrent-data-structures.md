...
| 6 | Безопасное освобождение памяти | 187–197 | ⚠️ Проверено с замечанием | 2026-05-08 | HP, EBR, RCU описаны корректно. **Замечание**: «ядро NetBSD (`vmem`)» как пример EBR — неточно. NetBSD имеет механизм passive serialization (pserialize), а `vmem` — это general-purpose resource allocator, не EBR-механизм. Корректнее упомянуть `pserialize`. |
...
| 11 | Список литературы | 287–329 | ⚠️ Проверено с замечаниями | 2026-05-08 | Все DOI и URL подтверждены, ссылки доступны. Замечания: (a) ссылка [19] на `lwlock.c` указывает не на тот файл — `WALInsertLock` array и `NUM_XLOGINSERT_LOCKS=8` определены в `src/backend/access/transam/xlog.c`, а не в `lwlock.c`; (b) [13] javadoc ConcurrentSkipListMap не содержит явного упоминания «Fraser и Harris» — это внутрикодовый комментарий в исходнике. |
